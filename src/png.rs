...
pub enum PngError {
    ...
    UnknownnChunkType(String),
    #[error("invalid header for PNG file")]
    InvalidHeader,
    #[error("source given is too small to be a valid PNG file")]
    TooSmall,
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        if value.len() < 8 {
            return Err(Error::new(PngError::TooSmall));
        }

        let mut chunks: Vec<Chunk> = Vec::new();
        let mut iter = value.iter().copied();
        let header: [u8; 8] = iter
            .by_ref()
            .take(8)
            .collect::<Vec<u8>>()
            .try_into()
        ...
        if Self::STANDARD_HEADER != header {
            return Err(Error::new(PngError::InvalidHeader));
        }
        ...
        while iter.len() >= 12 {
            ...
            let chunk_vec: Vec<u8> = iter.by_ref().take(12 + size as usize).collect();
            let chunk = Chunk::try_from(chunk_vec.as_slice())?;
...
impl Display for Png {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        ...
        let mut it = self.chunks.iter().peekable();
...
impl Png {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        Ok(Self::try_from(read(path)?.as_slice())?)
    }
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        for (_, chunk) in self.chunks.iter().enumerate() {
            if chunk.chunk_type().bytes() == chunk_type.as_bytes() {
                return Some(&chunk);
            }
        }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        self.header().iter().chain(bytes.iter()).copied().collect()
    }
}
